...
class HomeTadarusAdapter(private val listTadarus: List<Tadarus>): RecyclerView.Adapter<HomeTadarusAdapter.ViewHolder>() {
    inner class ViewHolder(private val binding: ItemTadarusHomeCardBinding): RecyclerView.ViewHolder(binding.root) {
        fun bind(item: Tadarus) {
            with(binding) {
                tvSurahName.text = item.surah
